...
const staticOutput = require('./out/static-output.js');
const interactiveUpdate = require('./out/interactive-update.js');
const updateAll = require('./out/update-all.js');
const debug = require('./state/debug.js');
const npmCheck = require('./index.js');
...
const SUPPORTED_INSTALLERS = new Set(['npm', 'pnpm', 'ied']);

function detectPreferredInstaller(cwd) {
    return detectPreferredPM(cwd)
        .then(preferredPM => preferredPM && SUPPORTED_INSTALLERS.has(preferredPM.name)
